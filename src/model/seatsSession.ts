 ...
 *
 * When the checkout opens, the count it prints falls by one or two seats while
 * the reader looks at it. Nobody took those seats. There is no reservation, no
 * write anywhere but this reader's own `sessionStorage`, and nothing any other
 * visitor can see: it is a display offset subtracted from a number that was
 * already invented (model/seats.ts, whose header says so at length).
 *
 ...
 * Three seats a session at most (two on the first open, one on the next, none
 * after), and never below 30 — a floor so an evening of reopening the sheet
 * can never empty the room. If the model's own count is already under the
 * floor, the offset takes nothing and the count is the model's.
 */
...
/** How many seats this session has taken so far, 0 to 3. */
...
/** What the next open takes: two on the session's first, one after, none
 *  once three are gone. */
